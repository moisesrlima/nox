        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-[var(--bg-surface)] p-6 rounded-2xl border border-[var(--border-color)] shadow-sm">
            ...
            <div className="flex items-center justify-center gap-4 mb-6">
              {!isTimerRunning ? (
                <button onClick={() => {
                  ...
                  <Play className="w-5 h-5" /> {pomodoroTime < (isBreak ? 5 * 60 : 25 * 60) ? 'Continuar' : (isBreak ? 'Iniciar Pausa' : 'Iniciar Foco')}
                </button>
              ) : (
            ...
            <div className="flex gap-4">
              <button onClick={() => { setIsBreak(false); setPomodoroTime(25 * 60); pauseTimer(); }} className={`flex-1 py-2 text-sm font-medium rounded-lg transition-colors ${!isBreak ? 'bg-[var(--accent-primary)]/10 text-[var(--accent-primary)]' : 'bg-[var(--bg-primary)] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]'}`}>
                25min Foco
              </button>
              <button onClick={() => { setIsBreak(true); setPomodoroTime(5 * 60); pauseTimer(); }} className={`flex-1 py-2 text-sm font-medium rounded-lg transition-colors ${isBreak ? 'bg-emerald-500/10 text-emerald-500' : 'bg-[var(--bg-primary)] text-[var(--text-secondary)] hover:bg-[var(--bg-hover)]'}`}>
                5min Pausa
              </button>
            </div>
          </div>
